...
- Использует mongoose в качестве базы данных
...
│   │   │
│   │   ├── statistics/ # Сущность статистики
│   │   │   ├── model/           # Модель статистики
│   │   │   │   ├── types.ts     # Типы данных
│   │   │   │   ├── api.ts       # API запросы
│   │   │   │   └── hooks.ts     # React хуки
│   │   │   └── ui/              # UI компоненты
...
│   │   ├── optionalAuthentication.mjs # Гибкая проверка аутентификации
...
2. **Entities (entities/)**
   ...
   - Примеры: User, Article, Statatistics
...
- **Страница деталей статьи** (`ArticleDetailsPage.tsx`)
  - Отображение полной информации о статье
  - Хлебные крошки для навигации
  - Индикаторы загрузки и обработка ошибок
  - Специальные функции для автора статьи:
    - Кнопка "Edit" для редактирования статьи
      - Открывает модальное окно редактирования
      - Стилизованная кнопка с анимацией при наведении
    - Кнопка "Delete article" для удаления статьи
      - Мгновенное удаление статьи
      - Стилизованная кнопка с анимацией при наведении
  - Кнопки управления видны только автору статьи
...
- Параметры запроса:
  ...
  - `content`: Содержание статьи (обязательное поле)
  - `img`: URL изображения (опциональное поле)
  - `userID`: ID автора статьи
...
- Возвращает:
  - Данные статьи
  - Флаг `isOwner` (true если текущий пользователь - автор)
- Включает информацию об авторе (username, \_id)
...
- Проверка прав доступа:
  - Сравнение ID автора статьи с ID текущего пользователя
...
- Проверка прав доступа:
  - Сравнение ID автора статьи с ID текущего пользователя
- Возвращает обновленную статью

#### Безопасность

- Все операции с изменением данных требуют аутентификации
...
- **optionalAuthentication.mjs**
  - Гибкий middleware для проверки аутентификации
  - Проверяет наличие токена в cookies
  - Если токен существует и валиден:
    - Прикрепляет данные пользователя к объекту запроса
  - Если токен отсутствует или невалиден:
    - Продолжает выполнение без данных пользователя
  - Всегда передает управление следующему обработчику
  - Используется для маршрутов, где аутентификация не обязательна
  - Позволяет предоставлять дополнительный функционал для авторизованных пользователей
